...
def search_restaurant_view(request):
    restaurant_id=request.GET.get('restuarant')
    selected_restaurant = get_object_or_404(Restaurant, pk=restaurant_id)
    if request.user.is_authenticated:
        user = auth.get_user(request)
        if user.is_anonymous():
            return render(request, 'restaurant.html', {'restaurant': selected_restaurant, })
...
def item_view(request):
    print("################### welcome ######################")
    item_id=request.GET.get('searchitem')
    print(item_id)
